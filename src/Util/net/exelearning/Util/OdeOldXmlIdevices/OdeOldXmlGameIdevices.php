...
/**
 * OdeOldXmlChallengeIdevice.
 */
class OdeOldXmlGameIdevices
{
    ...
    public static function oldElpGameIdevicesStructure($odeSessionId, $odePageId, $gameNode, $generatedIds, $nodeGameType, $xpathNamespace)
    {
        $result['odeComponentsSync'] = [];
        $result['srcRoutes'] = [];

        $gameNode->registerXPathNamespace('f', $xpathNamespace);
        // Get blockName
        $blockNameNode = $gameNode->xpath("f:dictionary/f:string[@value='_title']/following-sibling::f:unicode[1]/@value");

        $nodeIdevices = $gameNode->xpath("f:dictionary/f:list/f:instance[@class='exe.engine.field.TextAreaField']");
        foreach ($nodeIdevices as $nodeIdevice) {
            ...
            if ($nodeIdevice->{self::OLD_ODE_XML_DICTIONARY}->{self::OLD_ODE_XML_UNICODE}) {
                ...
                // Set type
                $odeComponentsSync->setOdeIdeviceTypeName($nodeGameType);
