    ...
    public static function oldElpFileAttachIdeviceStructure($odeSessionId, $odePageId, $galleryImageNodes, $generatedIds, $xpathNamespace)
    {
        ...
        foreach ($galleryImageNodes as $galleryImageNode) {
            ...
            foreach ($files as $file) {
                ...
                $commonReplaces = [
                    '{{image_path}}' => $sessionPath.$odeIdeviceId.Constants::SLASH.(string) $filePath[0]['value'],
                    // "{{thumb_path}}" => $sessionPath . $odeIdeviceId . Constants::SLASH . (string) $imagePath[1]["value"]
                ];

                $fileInfo['path'] = $fullImagePath;
                $fileInfo['description'] = $fileDescription[0]['value'];

                if ('' == $fileInfo['description']) {
                    $fileInfo['description'] = $filePath[0]['value'];
                }
            ...
            foreach ($jsonImages as $key => $jsonImage) {
                $odeComponentsSyncHtmlView .= '<p><a href="'.$jsonImage['path'].'" target="_blank">'.$jsonImage['description'].'</a></p>';
            }
